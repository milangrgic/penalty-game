...
import { displayErrorMessage, getTransactionRequests } from '../../utils/helper';
...
const MyRequests = () => {
  ...
  const fetchMyRequests = async () => {
    ...
    const myRequests = allRequests.filter((request) => (request.from === web3Ctx.account));
    setRequests(myRequests);
  }

  const completeTransferHandler = async (id) => {
    try {
      await web3Ctx.contracts.community.methods.completeTransfer(id).send({from: web3Ctx.account});
                        ...
                        {requests.map((request, i) => (
                          ...
                          >
                            ...
                            <TableCell align='center'>
                              {request.approved && !request.completed && <CLoadingButton color='success' onClickHandler={() => completeTransferHandler(request.id)} title='Complete' />}
                            </TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
